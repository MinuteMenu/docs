...
**Status:** ✅ Confirmed by Jay Peters on [TP 323019 / comment 430027](https://minutemenu.tpondemand.com/entity/323019). Proceed with separate tables.
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| Q3a | After we save a walk-in child, what does the center do with that record? Two paths. **(a)** A one-time audit trail — a name on the sign-in sheet, kept for compliance, never reused. **(b)** A working record the center reuses across days and weeks. We need the answer because the two paths need different storage shapes and different upkeep. Feeds §7.1. [TP 323019 comment 429994](https://minutemenu.tpondemand.com/entity/323019) | ✅ Settled by the §7.1 separate-table decision ([Jay's reply, TP 323019 c/430027](https://minutemenu.tpondemand.com/entity/323019)). Walk-in records are minimal-information by design; treated as their own thing, not part of the registered-child lifecycle. | confirmed |
| Q3b | Can a walk-in child be converted to a full-enrolled child later? Yes or no. If yes, briefly describe what the center expects — same record with fields filled in, or a new enrollment that supersedes the walk-in? Feeds §7.1. [TP 323019 comment 429994](https://minutemenu.tpondemand.com/entity/323019) | Not directly addressed by Jay's reply. With separate-table storage, promotion would mean an explicit copy from the hybrid table into `CHILD`. Defer to product if/when conversion becomes a real workflow need. | asked |
| Q3c | What does CACFP/ARAS/SFSP regulation actually require of these records — just a name on the sign-in sheet, or the same audit-trail compliance as enrolled kids? Drives how much compliance scaffolding hybrid records need. Feeds §7.1. [TP 323019 comment 429994](https://minutemenu.tpondemand.com/entity/323019) | ✅ Implicit in [Jay's reply, TP 323019 c/430027](https://minutemenu.tpondemand.com/entity/323019): "both use minimal information when compared to CHILD and CLAIM_ATTENDANCE." Minimum info is sufficient; no full audit-trail scaffolding required. | confirmed |
| Q3d | Where do hybrid kids show up in reports? Alongside full-enrolled kids in sponsor monthly reports and headcount dashboards, or in a separate hybrid-only view? If mixed, reporting queries have to know about both worlds. Feeds §7.1. [TP 323019 comment 429994](https://minutemenu.tpondemand.com/entity/323019) | Not directly addressed by Jay's reply. Separate-table storage means hybrid kids do *not* appear in existing report queries by default. If product later wants them mixed into existing reports, those queries will need an explicit UNION. | asked |
...
| Q19 | How does the system identify a hybrid site in code? Today's Open Enrolled detection uses `program_type_code IN (1614, 2218) AND no active CHILD rows` because legacy codes are overloaded. Hybrid uses dedicated codes — should detection just check the program type, with no child-count heuristic? **(NEW — to review at meeting.)** [TP 323015 c/430046](https://minutemenu.tpondemand.com/entity/323015) | _(Phan proposes)_ Yes — identify hybrid sites purely by `program_type_code IN (AtRiskHybrid, SFSPHybrid)`. No child-count check. The existing Open Enrolled detection stays unchanged and the two paths do not overlap. | asked |
...
| Q20 | When a center already has an Open Enrolled license, should a Hybrid license of the *same program type* be allowed on the same center? Production data: 35 centers hold OE+Regular mixes; 1 of 35 has the same-program-type-in-two-modes shape. **(NEW — to review at meeting. May belong in Highest priority.)** [TP 323015 c/430046](https://minutemenu.tpondemand.com/entity/323015) | _(Phan proposes)_ Option B — allow hybrid except when the same program type already has Open Enrolled on the same center. Blocks ARAS OE + ARAS Hybrid and SFSP OE + SFSP Hybrid; allows CACFP + Hybrid, ARAS Hybrid + SFSP Hybrid, etc. UI guard at License Selection + server-side mirror. | asked |
| Q10 | Can a walk-in child be later "promoted" to full enrollment without losing attendance history? Same `child_id`?                                  |        | draft  |
| Q21 | Hybrid kids accumulate in `OPEN_ENROLLED_CHILD` over weeks and months. How should the ATMC roster handle this — show everyone forever, show recent attendees only, show yesterday only, or start blank each day? And if recent-only, what threshold? **(NEW — to review at meeting.)** [TP 323015 c/430046](https://minutemenu.tpondemand.com/entity/323015) | _(Phan proposes)_ Option B — show children who attended within the last N days, default 30. Self-cleaning, regulars stay visible, occasional attendees re-appear cleanly when they return. Threshold value is a UX call. | asked |
| Q11 | Do hybrid sites appear in CX Admin (WinForms) like ARAS/SFSP sites today, including annual renewal workflows?                                     |        | draft  |
| Q22 | Which existing screens should hybrid centers see, and which new screens are hybrid-only? Phan proposes widening `IsOpenEnrollCenter` / `IsOpenEnrollCenterNonLA` to cover hybrid (so existing OE-gated screens automatically work) and adding new `IsARASHybrid` / `IsSFSPHybrid` flags for the brand-new screens. A 10-row visibility matrix is on TP. **(NEW — to review at meeting. Confirm matrix row-by-row, especially "Delivery / pickup screens visible for hybrid".)** [TP 323015 c/430046](https://minutemenu.tpondemand.com/entity/323015) | _(Phan proposes)_ Widen the existing OE flags to also return true for hybrid licenses. Add new `IsARASHybrid` and `IsSFSPHybrid` flags for the new screens only. Existing screens stay visible to hybrid by default. See TP comment for the full matrix. | asked |
...
| Q14 | The spec says "ARAS Hybrid school-calendar checks: same as ARAS." But `ProcessArassfsp()` (the claim path used by open-enrolled and the count-based path) does not run school-calendar checks today — those live in the per-child ChildChecker path used by closed-enrolled ARAS, and they read child properties (`SchoolTypeCode`, `SchoolDistrictId`, `SchoolName`) that hybrid lightweight kids do not capture. What should hybrid do — skip these checks entirely, add a hybrid-specific version into `ProcessArassfsp()`, or something between? [TP 323015 comment 429998](https://minutemenu.tpondemand.com/entity/323015) | ✅ Confirmed by Jay ([TP 323015 c/430029](https://minutemenu.tpondemand.com/entity/323015)): no school-calendar check for hybrid. Retain the current ARAS claims-processing behavior. Lightweight kids may lack DOB and definitely lack school data, so the checks would have nothing to read. *Note: this also resolves the spec ambiguity — the "same as ARAS" line meant "same as current ARAS claims processing", which on the count-based path means no school-calendar check.* | confirmed |
