...
|----|----------|--------|--------|
| Q2 | Lightweight Enrollment vs Walk-in — are they two distinct paths or one path with a "mark attendance now" toggle? Drives whether stories 323016 and 323019 share an endpoint. | _(proposed)_ Pre-enroll = no auto-attendance; Walk-in = auto-mark attendance + current meal. Two distinct paths that share the same lightweight create logic. | draft |
| Q3 | Should walk-in children carry a distinct flag in the database (so reports can separate "pre-enrolled hybrid" from "walked in mid-meal")? If yes, this adds a CXADMIN schema migration not currently in any story. | | draft |
| Q15 | Will product accept a new route `/centers/children` for lightweight enrollment, or should it sit on the existing `/child` prefix? Affects FE contract. | | draft |
