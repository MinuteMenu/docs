...
- **CHECK_REGISTER covering index (#316306)** — new non-clustered index with 7 INCLUDE columns on a multi-million-row table. CREATE NONCLUSTERED does not specify `ONLINE = ON` → Sch-M lock for duration of build, blocks all reads/writes to CHECK_REGISTER.
  **Action required** — run DDL in maintenance window; add `WITH (ONLINE = ON, MAXDOP = 4)` if Enterprise Edition.
  Rollback: Low — standalone ROLLBACK script exists at `CXADMIN/ROLLBACK/316306_create_check_register_client_center_covering_index.sql`.
  ...
  Evidence: Pattern #5 scope-creep risk — non-SARGable query `Year(cr.check_date)=Year(@claim_month)` in `select_payment_history_list_sp` still scans client-wide even with the covering index.
...
- **Orphan detect/cleanup endpoints (#319630)** — `common/cleanupOrphanedEforms` with auto-detect (null `ClientIds`) performs serial per-client fan-out: opens fresh eFormsContext + CxContext per iteration and pulls entire active CHILD roster per client. No `CommandTimeout`. No row cap.
  **Action required** — (1) require non-empty `ClientIds` in prod (reject auto-sweep), (2) add explicit `Database.CommandTimeout = 30-60s`, (3) add `[RequiredRole]` on top of security-key check, (4) firewall to DevOps app pool / IP allow-list, (5) audit-log every invocation (caller, ClientIds, row counts).
  Rollback: Low — disable endpoint via IIS route rule or back-out code.
  ...
  Evidence: Matches known incident patterns 3/4 (connection accumulation, thread-pool starvation, elevated CommandTimeout). **Also: orphan predicate treats Withdrawn/Inactive children as orphans → hard-deletes their historical eForms. Recommend NOT running in auto-sweep mode on prod until predicate is tightened.**
...
- **Sponsor 102 PrintIssuePayment fan-out (#320213)** — HX API controller uses unbounded `Task.WhenAll` calling `BuildSponsor102Data` per check; each call fires 5 stored procedures in parallel. A 500-provider batch print = 2,500 concurrent DB round-trips.
  **Monitor** — spike in MMADMIN connection count during first post-release 102 print.
...
- **MRP_Rollback scope gap** — `CXADMIN/MRP_Rollback.build.config` only covers `select_payment_history_list_sp`. Rollbacks for 318686 policy INSERT, `select_rpt_meal_count_attendance_sp`, `select_361127_Child_Reconciliation_sp`, `select_rpt_401_child_enrollment_sp` are NOT in the bundled rollback.
  **Action required** — DBA must hand-craft rollback for those SPs + DELETE policy row if rollback is triggered.
  Rollback: Medium — partial automated, partial manual.
