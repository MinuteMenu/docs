...
The login flags become OR'd — `isARASHybrid = true` if *any* license on the center is `AtRiskHybrid`. The model does not say what happens when the same child attends both programs. See §7.1 and Q3e.
...
**Open dependencies on product (see §8 Q3a–Q3e).** Five product questions feed this recommendation:
...
- **Q3e — Cross-program identity.** Same child in both programs — one record or two?

If product answers point to "durable identity, frequent promotion, mixed reporting, shared identity across programs", same-table starts to argue for itself and we revisit.
...
| Q3e | A center can hold both a regular CACFP license and an ARAS Hybrid license at the same site — morning daycare and afterschool program on the same site is a real example. If the same child attends both programs, do we keep one record linked to both, or two independent records? Strongly affects whether full and hybrid storage can share a key. Feeds §7.1. | | draft |
